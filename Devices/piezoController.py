# -*- coding: utf-8 -*-
"""
Created on Fri Dec 13 14:22:00 2024

@author: Johannes Eberle (taken from Github)
...
## Implementing it implies change to the code, to parse the echo response
    ...
    def set_SCx(self, target_voltage, step_size=0.1, delay=0.05, thr_voltage=0.75):
        # previously named set_x_voltage
        """
        Smoothly sets x voltage from its current value to the target value.
    
        Parameters:
            target_voltage (float): The desired voltage value.
            step_size (float): The increment size for each step.
            delay (float): The time (in seconds) to wait between each step.
        """
        current_voltage = self.get_SCx()
        if abs(current_voltage-target_voltage) < thr_voltage:
            self._send_command(self._voltage_commands_set[0], target_voltage)
        else:
            direction = 1 if target_voltage > current_voltage else -1
            total_steps = int(abs(target_voltage - current_voltage) / step_size)
            for _ in range(total_steps):
                current_voltage += direction * step_size
                self._send_command(self._voltage_commands_set[0], current_voltage)
                time.sleep(delay)
            self._send_command(self._voltage_commands_set[0], target_voltage)
    
    def set_SCy(self, target_voltage, step_size=0.1, delay=0.05, thr_voltage=0.75):
        """
        Smoothly sets y voltage from its current value to the target value.
        ...
        Parameters:
            ...
            delay (float): The time (in seconds) to wait between each step.
        """
        current_voltage = self.get_SCy()
        if abs(current_voltage-target_voltage) < thr_voltage:
            self._send_command(self._voltage_commands_set[1], target_voltage)
        else:
            direction = 1 if target_voltage > current_voltage else -1
            total_steps = int(abs(target_voltage - current_voltage) / step_size)
            for _ in range(total_steps):
                current_voltage += direction * step_size
                self._send_command(self._voltage_commands_set[1], current_voltage)
                time.sleep(delay)
            self._send_command(self._voltage_commands_set[1], target_voltage)
